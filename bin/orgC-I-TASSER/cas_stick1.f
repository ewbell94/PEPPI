...
c      open(unit=28,file='comb8CA.dat',status='unknown') !CA-contact at 8A
...
cccccccccccccccc sequence-based contact predictions --------------->
      open(unit=50,file='conf5.comm',status='unknown')
      open(unit=51,file='svmseq.dat',status='unknown') !1, CA
      open(unit=52,file='svmcon.dat',status='unknown') !2, beta
      open(unit=53,file='betacon.dat',status='unknown') !3, CA
      open(unit=54,file='psicov.dat',status='unknown') !4, beta
      open(unit=55,file='spcon.dat',status='unknown') !5, beta
      open(unit=56,file='metapsicov.dat',status='unknown') !6, beta
      open(unit=57,file='nnbayes.dat',status='unknown') !7, CA
      open(unit=58,file='nnbayesb.dat',status='unknown') !8, beta
...
c      call read_CAcontact8      !read CAcontact restrains from 'comb8CA.dat'
...
c     CAcontact restrain is only on CA.
      ...
      n_conf=8                  !number of programs for contact predictions
      acc_cut=acc_cut+0.01      !shift one bar
      
ccc   read conf.comm ------->
 10   read(50,*)acc0
c      write(*,*)'acc0=',acc0,acc_cut
...
c            write(*,*)k1,conf0(k1,k2),dist,it
c     adjust conf0:
            if(it.eq.2)then     !CB
               conf0(k1,k2)=conf0(k1,k2)*acc_cutB !acc_cutB=1.1
            endif
            if(it.eq.3)then     !SG
               conf0(k1,k2)=conf0(k1,k2)*acc_cutG !acc_cutG=1.1
            endif
...
c     record number of contact type:
            if(k1.eq.1)then     !CA
               ...
               ik_svm(it,nk_svm(it))=k2 !type of contact (n_conf contacts)
c               write(*,*)k1,k2,it,nk_svm(it),ik_svm(it,nk_svm(it))
            endif
         enddo
      enddo
...
c      do i=1,3
c         do j=1,nk_svm(i)
c            write(*,*)i,j,ik_svm(i,j),nk_svm(i),'---'
c         enddo
c      enddo
c      do i=1,n_conf
...
ccc   decided based on table at /home/yzhang/pdbinput/contact/readme
      nc05=Lch*0.05             !total is Lch*0.15
      nc10=Lch*0.1              !total is Lch*0.3
      nc17=Lch*0.17             !total is Lch/2=Lch*0.51
      nc20=Lch*0.2              !total is Lch*0.6
      do k1=1,3                 !short/medm/long
         do k2=1,n_conf         !betacon, SPcon etc
            mmm(k1,k2)=nc10     !remove over-trains, at least 0.1L to use for each catigory
         enddo
      enddo
...
ccc   read contact predictions ------->
      do k=1,n_conf
         ...
         read(50+k,*,end=14)ntmp !if contact file not exist, no contact will be used
...
c     decide if take the contact prediction
            mk=0                !donot take
            if(nnn(jco,k).lt.mmm(jco,k))then !at least 0.1L for each CO, each program
               mk=1
            elseif(i_c.lt.nc20)then !at least 0.2Lch for each program
               mk=1
...
c     record contact and weight
            if(mk.eq.1)then
               ...
               r1(i_c)=i1
               r2(i_c)=i2
               awei(k,i1,i2)=2.5*(1+(conf-conf0(jco,k)))*aw_cont
...
ccc   map r1,2(i) into Mcon(k,i),Kcon(k,i,Mcon(i))------------>
         ...
         goto 130
c     output restraints------------->
         write(*,*)'type, acc0,cutoff,conf_short,medm,long=',k,acc0,
     &        conf0(1,k),conf0(2,k),conf0(3,k)
 ...
 13         format(i4,'(',i2,'):',20f8.5)
         ...
         write(*,*)'N_short=(n_tot,mim,type)',nnn(1,k),mmm(1,k),k
         write(*,*)'N_medm =(n_tot,mim,type)',nnn(2,k),mmm(2,k),k
         write(*,*)'N_long =(n_tot,mim,type)',nnn(3,k),mmm(3,k),k
         write(*,*)
 130     continue
      enddo
...
c     combCA8.dat--------->
c            do k=1,McomCA8(j)
c               m=KcomCA8(j,k)   !k'th contact with j
c               if(m.gt.j)then
c                  t_combCA8=t_combCA8+1
c                  dis=(x(j)-x(m))**2+(y(j)-y(m))**2+(z(j)-z(m))**2
c                  if(dis.le.dist_CA_cut8)then
c                     s_combCA8=s_combCA8+1
c                  endif
c               endif
c            enddo
ccc   
