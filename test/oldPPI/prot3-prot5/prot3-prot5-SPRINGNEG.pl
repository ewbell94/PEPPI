...
my $complexlist="$dbdir/70negs.txt";
...
my $user=`whoami`;
chomp($user);
my @weights=(1,12.0,0.0); #weights for SPRING score calculation
my $homothresh=0.9; #
...
#Copy HHR files or run HHsearch
    ...
    print `$bindir/dimerMapperNeg $tempdir/$prot1.hhr $tempdir/$prot2.hhr`;
