...
my $pairname="prot2-prot5";
my $benchmarkflag=0;
my $starttime=time();

print `mkdir -p $outdir/$pairname/SPRINGNEG`;

#User-set parameters
my $bindir="$peppidir/bin"; #location of program binaries
my $springout="$outdir/$pairname/SPRINGNEG"; #location of program output
my $dbdir="/nfs/amino-home/ewbell/SPRINGDB"; #location of SPRING database
my $complexlist="$dbdir/70negs.txt"; #location of pairs in the database
...
my $user=`whoami`;
chomp($user);
my @weights=(1,7.6); #weights for SPRINGNEG score calculation
my @scorerange=(0.0,67.0); #minimum and maximum possible score
my $dimercount=10; #number of dimers to be assembled
my $topcount=$dimercount; #number of top dimers to report in TemplateSummary.txt
my $topscore=-100.0; #Top score to write into res.txt
...
my $tempdir="/tmp/$user/PEPPI_SPRINGNEG_$qseqs[0]-$qseqs[1]\_$randomTag";
if (! -e "$tempdir"){
...
for my $pointer (@domainPairs){
	...
	print $summary sprintf("%s\t%s\t%.3f\t%.3f\t%.3f\n",$dimerModels[$i][0],$dimerModels[$i][1],$dimerModels[$i][2][0],$dimerModels[$i][2][1],$dimerModels[$i][2][2]);
    }
    close($summary);
...
if ($topscore!=-100.0){
    if ($topscore < $scorerange[0]){
        print "Best SPRINGNEGscore is outside of training set range.  Set to $scorerange[0] for classification purposes\n";
        $topscore=$scorerange[0];
    } elsif ($topscore > $scorerange[1]){
        print "Best SPRINGNEGscore is outside of training set range.  Set to $scorerange[1] for classification purposes\n";
        $topscore=$scorerange[1];
    }
    while (`lsof $outdir/SPRINGNEGres.txt | wc -l` > 0){ #To prevent simultaneous file writing
	sleep(1);
    }
    print `echo "$pairname,$topscore" >> $outdir/SPRINGNEGres.txt`;
}

open(my $resfile,">","$outdir/$pairname/SPRINGNEG/res.txt");
...
sub constructModel{
    ...
    my @scores=($springscore,$zscore,$tmscore);
    return @scores;
}
