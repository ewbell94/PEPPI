...
my @qchains=split("-",$pairname);
print `mkdir $outdir/$pairname/SPRING`;

#User-set parameters
my $bindir="$peppidir/SPRING-PPI/bin"; #location of program binaries
my $outputdir="$outdir/$pairname/SPRING"; #location of program output
...
my $scut=1.1; #monomeric sequence homology cutoffs for threading; 0.3="benchmark", 1.1="real"
...
my $hhdir="$outputdir/hhr"; #location of hhr files of previously run HHsearch results
...
my $currdir=`pwd`;
chomp($currdir);
my $prot1file="$outdir/$pairname/$qchains[0].seq";
$prot1file="$currdir/$prot1file" if (substr($prot1file,0,1) ne "/");
my $prot2file="$outdir/$pairname/$qchains[1].seq";
...
if (! -e "$outdir/$pairname/SPRING/TemplateSummary.txt"){
    print $resfile "?\n";
} else {
    open(my $springres,"<","$outdir/$pairname/SPRING/TemplateSummary.txt");
    my $toptemplate=<$springres>;
    close($springres);
    my @parts=split(' ',$toptemplate);
    print $resfile "$parts[2]\n";
}
