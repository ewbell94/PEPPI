...
my $minmono=100; #minimum number of monomer templates for dimer matching
...
my @prot1hits=newFetchHits($prot1);
my @prot2hits=newFetchHits($prot2);
...
=pod
	...
	#my $prescore=$parts[4];
	#$prescore=1e-300 if ($parts[4]==0.0);
	#push(@scores,-log($prescore));
	#push(@scores,$parts[5]/$parts[7]);
	push(@scores,$parts[5]+$parts[6]);
	#push(@scores,$parts[2]);
    }
=cut
    while (my $line=<$hhrfile>){
	...
	if ($pairs[$i][1]<$zthresh){
	    if (scalar(@outlist)<$minmono){
		$zthresh--;
	    } else {
		last;
	    }
